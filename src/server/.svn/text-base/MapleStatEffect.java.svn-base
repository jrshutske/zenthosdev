...
public class MapleStatEffect implements Serializable {
        ...
        if (skill) {
            switch (sourceid) {
                ...
                case Aran.FINAL_CROSS:
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.WATK, x));
                    break;
                case Aran.COMBO_DRAIN:
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.COMBO_DRAIN, Integer.valueOf(x)));
                    break;
                case Aran.BODY_PRESSURE:
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.BODY_PRESSURE, Integer.valueOf(ret.y)));
                    break;
                ...
                case Marauder.TRANSFORMATION:
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MORPH, Integer.valueOf(x)));
                    break;
                case Aran.COMBO_BARRIER:
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.COMBO_BARRIER, Integer.valueOf(x)));
                    break;
                case ThunderBreaker.SPARK:
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SPARK, 1));
                    break;
                default:
                    break;
            }
    ...
    private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
        ...
        if (summonMovementType != null && pos != null) {
            ...
            if (!tosummon.isPuppet()) {
                applyfrom.getAntiCheats().resetSummonAttack();
            }
        ...
        if (isPartyBuff() && (applyfrom.getParty() != null || isGmBuff())) {
            ...
            for (MapleMapObject affectedmo : affecteds) {
                ...
                if (affected != null && applyfrom == null && isHeal() && affected != applyfrom && affected.getParty() == applyfrom.getParty() && affected.isAlive()) {
                    int expadd = (int) ((calcHPChange(applyfrom, true) / 10) * (ServerProperties.getExpRate + ((Math.random() * 10) + 30)) * (Math.floor(Math.random() * (applyfrom.getSkillLevel(SkillFactory.getSkill(2301002))) / 100) * (applyfrom.getLevel() / 30)));
    ...
    public void silentApplyBuff(MapleCharacter chr, long starttime) {
        ...
        if (summonMovementType != null) {
            ...
            if (!tosummon.isPuppet()) {
                ...
                chr.getAntiCheats().resetSummonAttack();
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
        ...
        if (localstatups.size() > 0) {
            if (isDash()) {
                if (applyto.getJob().isA(MapleJob.PIRATE) || applyto.getJob().isA(MapleJob.THUNDERBREAKER1)) {
...
//            applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this), false);
    ...
    private boolean isPartyBuff() {
        ...
        if ((sourceid >= 1211003 && sourceid <= 1211008) || sourceid == Paladin.SWORD_HOLY_CHARGE || sourceid == Paladin.BW_HOLY_CHARGE || sourceid == DawnWarrior.SOUL_CHARGE) {
            return false;
        }
        return true;
    }
    ...
    public boolean isHide() {
        return skill && sourceid == SuperGM.HIDE;
    }
    ...
    public boolean isMonsterRiding() {
        return skill && (sourceid == 1004 || sourceid == Corsair.BATTLE_SHIP);
    }
