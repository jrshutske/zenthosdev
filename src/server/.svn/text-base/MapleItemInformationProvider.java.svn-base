...
public class MapleItemInformationProvider {
    ...
    protected final MapleData cashStringData = MapleWZProvider.stringWZ.getData("Cash.img");
    protected final MapleData consumeStringData = MapleWZProvider.stringWZ.getData("Consume.img");
    protected final MapleData eqpStringData = MapleWZProvider.stringWZ.getData("Eqp.img");
    protected final MapleData etcStringData = MapleWZProvider.stringWZ.getData("Etc.img");
    protected final MapleData insStringData = MapleWZProvider.stringWZ.getData("Ins.img");
    protected final MapleData petStringData = MapleWZProvider.stringWZ.getData("Pet.img");
    protected final MapleDataProvider equipData = MapleWZProvider.equipWZ;
    protected final MapleDataProvider itemData = MapleWZProvider.itemWZ;
    ...
    protected Map<Integer, List<MapleFish>> fishCache = new HashMap<Integer, List<MapleFish>>();
    ...
    public List<Pair<Integer, String>> getAllItems() {
        ...
        itemsData = eqpStringData.getChildByPath("Eqp");
        ...
        itemsData = etcStringData.getChildByPath("Etc");
        for (MapleData itemFolder : itemsData.getChildren()) {
            int itemId = Integer.parseInt(itemFolder.getName());
            String itemName = MapleDataTool.getString("name", itemFolder, "NO-NAME");
            itemPairs.add(new Pair<Integer, String>(itemId, itemName));
    ...
    public Equip randomizeStats(MapleClient c, Equip equip) {
        short x = 1;
        if (ServerProperties.isGodlyItems && Math.ceil(Math.random() * 100.0) <= ServerProperties.getGodlyItemRate) {
            x = ServerProperties.getItemStatMultiplier;
        }
    ...
    public List<MapleFish> getFishReward(int itemId) {
        if (fishCache.containsKey(itemId)) {
            List<MapleFish> sorted = fishCache.get(itemId);
