    ...
    public static void execute(MapleClient c, String[] splitted, String command) throws Exception {
        ...
        if (command.equals("buffme")) {
            ...
            if (splitted.length < 2) {
                player.msg("Syntax: !goto <mapname> <optional_target>, where target is char name and mapname is one of:");
                ...
                if (maps.containsKey(message)) {
                    ...
                    player.msg("Could not find map");
                }
            }
            maps.clear();
        } else if (command.equals("sexchange")) {
            if (player.getGender() == 1){
                player.setGender(0);
            } else {
                player.setGender(1);
            }
            player.msg(player.getName() + " sex change!");
        } else if (command.equals("storage")) {
            c.getPlayer().getStorage().sendStorage(c, 2080005);
        } else {
            System.out.println("Donator Command: !" + command + " does not exist.");
        }
    }
}