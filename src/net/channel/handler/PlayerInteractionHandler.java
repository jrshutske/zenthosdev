    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) {
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    if (c.getPlayer().haveItem(itemId, 1, false, true)) {
                        ...
                        AntiCheats.autoBan(c, "Merchant Shop: Attempt to open a shop without the item");
                        return;
                    }
                } else {
            ...
            if (c.getPlayer().getTrade() != null && item != null) {
                if (checkq > 4000) {
                    AntiCheats.autoBan(c, "PE while in trade");
                    return;
                }
                if ((quantity <= item.getQuantity() && quantity >= 0) || InventoryConstants.isRechargable(item.getItemId())) {
                    if (!ServerProperties.allowUndroppablesDrop && ii.isDropRestricted(item.getItemId())) {
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
                    if (price < 0) {
                        AntiCheats.autoBan(c, "PE adding a negative priced item");
                        return;
                    }
                    if (bundles <= 0 || perBundle <= 0 || checkquantity > 20000 || checksmquantity < 0 || checkiquantity < 0 || checkiquantity > 20000) {
                        AntiCheats.autoBan(c, "PE Player Shop item: " + sellItem.getItemId());
                        return;
                    }
            ...
            if (!shop.isOwner(c.getPlayer())) {
                AntiCheats.autoBan(c, "PE Attempting to remove item from: " + shop.getOwnerName());
                return;
            }
            if (shop != null) {
