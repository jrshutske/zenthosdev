...
function announce() {
	...
	em.getChannelServer().broadcastPacket( 
		Packages.tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement will open in " + i + " minutes"));
	i--;
}
...
function start() {
	scheduleNew();
	em.getChannelServer().broadcastPacket( 
		Packages.tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement is now open"));	
	var iter = em.getInstances().iterator();
	while (iter.hasNext()) {
		var eim = iter.next();
		if (eim.getPlayerCount() > 0) {
...
function startInstance(eim) {
	if (eim.getPlayerCount() > 0) {
		...
		if (eim.getName().equals("lolcastle1")) {
			for (var x = 0; x < 100; x++) {
				var mob = Packages.server.life.MapleLifeFactory.getMonster(9500169);
			...
			for (var x = 0; x < 10; x++) {
				var mob = Packages.server.life.MapleLifeFactory.getMonster(9300136);
				var overrideStats = new Packages.server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 100; x++) {
				var mob = Packages.server.life.MapleLifeFactory.getMonster(5120000);
				var overrideStats = new Packages.server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 3; x++) {
				var mob = Packages.server.life.MapleLifeFactory.getMonster(9300012);
				var overrideStats = new Packages.server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 3; x++) {
				var mob = Packages.server.life.MapleLifeFactory.getMonster(9300105);
				var overrideStats = new Packages.server.life.MapleMonsterStats();
				overrideStats.setHp(mob.getHp());
				overrideStats.setExp(mob.getExp() / 4);
				overrideStats.setMp(mob.getMaxMp());
			...
			for (var x = 0; x < 3; x++) {
				var mob = Packages.server.life.MapleLifeFactory.getMonster(9300152);
				var overrideStats = new Packages.server.life.MapleMonsterStats();
...
function allMonstersDead(eim, player) {
	...
	if (priceRand <= 35) {
		...
		*/
		...
		price = new Packages.client.Item(starId, 0, ii.getSlotMax(player.getClient(), starId));
