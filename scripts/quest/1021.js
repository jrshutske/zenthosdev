...
function start(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			qm.sendNext("Hey, Man~ What's up wit it? Haha! I am Roger who can teach you adorable new Maplers lots of information.");
		} else if (status == 1) {
			qm.sendNextPrev("You are asking who made me do this? Ahahahaha!\r\nMyself! I wanted to do this and just be kind to you new travelers.");
		} else if (status == 2) {
			qm.sendAcceptDecline("So..... Let me just do this for fun! Abaracadabra~!");
		} else if (status == 3) {
			if (qm.getC().getTimesTalked() == 0) {
